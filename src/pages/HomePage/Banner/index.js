...
class Banner extends Component {
  ...
  componentDidMount() {
    this.getHomePageMiddleBannerById();
  }

  // declare home page middle banner function
  getHomePageMiddleBannerById = () => {
    const url = "http://breed-dev-back.vuwork.com:8082/getBannerById/";
    const BannerId = 3;
    httpGet(url, BannerId)
      .then((response) => {
        this.setState({
          home_page_middle_banner: response.data,
        });
      })
    ...
    render() {
      const { home_page_middle_banner } = this.state;
      return (
        <>
         <Box
        ...
        >
          <Container style={{ maxWidth: "1208px" }}>
            <Box>
              ...
              <Link href={home_page_middle_banner.link}>
              <img
                  src={home_page_middle_banner.image}
                  width="100%"
                  alt="banner-img"
                />
              </Link>
